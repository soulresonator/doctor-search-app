...
- **Task 1.3**: Create a User Profile page (View & Edit). -> Finished, added avatar image upload functionality to the user profile.
...
- **Task 2.4**: Create a rich Booking Modal showing doctor info and allowing Date/Time selection. -> Finished, added quick book buttons for "Tomorrow" and "Next Weekend", and also limited the max booking date to 1 year in the future.
...
- **Task 4.1**: Build a terminal UI app (TUI) to wrap `manage.py` commands  (migrations, superuser, runserver) for easier backend management. -> Finished, added commands to reset the database and seed it with mock data.
...
- For developing process, we need a command to reset the database (drop all tables and create all tables again), and a command to reset the database and seed with mock data for testing purposes. -> Finished.

- In the next Sprint, we need to apply CI/CD for auto testing for this app. -> A baseline GitHub Actions CI workflow was set up in Sprint 02 to pave the way.
